...
The first path is the initial root. Press `Tab` to switch roots. Multi-root root selection and locations exist only while the Overlay is open; `restoreBrowsePosition` remains a single-root-only feature.
