...
- Browse-position restoration is disabled by default. Users can opt in through the global Pi settings file, which the extension reads but never writes:
  ...
  With that setting in Pi's global settings file (`$PI_CODING_AGENT_DIR/settings.json`, defaulting to `~/.pi/agent/settings.json`), closing a no-argument single-root browser records its root, selected file, and that file's directory in extension memory. A no-argument reopen preserves the recorded root for `.` while locating the file selection or directory within it; the header briefly shows `↳ restored: <path>` until the next input. If the file is gone but its directory remains, it restores that directory. An explicit path overrides restoration only for that invocation and never replaces the remembered no-argument position. This state is temporary for the Pi process, and a missing root or directory falls back to the default root.
