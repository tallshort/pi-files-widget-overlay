...
- `/` opens viewer search; pressing `/` again clears the query, `Esc` always cancels it, and `Backspace` cancels when the query is empty. `?` toggles between a curated one-line hint and complete two-line viewer keybindings.
