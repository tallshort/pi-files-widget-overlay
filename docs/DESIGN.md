...
- Closing the browser records its root, selected file, and that file's directory in extension memory. A no-argument reopen restores the file selection or directory when the paths remain valid; if the file is gone but its directory remains, it restores that directory. Explicit paths override it. This state is temporary for the Pi process, and a missing root or directory falls back to the default root.
