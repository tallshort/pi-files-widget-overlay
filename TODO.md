...
- [x] Restore the first root's memory-only browse position when `/readfiles` receives multiple roots; if no valid record exists, start at that first root. Keep single-root lookup unchanged, bypass restoration for an explicit single `/readfiles <path>`, and do not persist state or add dependencies.
- [ ] **Blocked:** Add optional inline image previews for supported terminal protocols using Pi `>=0.84.4`. A prototype is retained on `feature/image-preview-overlay-prototype`, but Pi's floating-overlay compositor truncates iTerm2 OSC 1337 image payloads, so it produces an empty image. Keep the safe metadata placeholder until Pi supports image protocol payloads in overlays; then enforce the planned 10 MiB (`10 * 1024 * 1024` bytes) source limit and conservative rendered dimensions.
...
- [x] Add `*` to pin/unpin the selected directory (or a selected file's parent) as a persistent root. Store de-duplicated, lexically normalized absolute user-visible paths (resolve `..` and trailing slashes but do not `realpath`; symlink paths and targets may be pinned separately) in global `piFilesWidgetOverlay.pinnedRoots`, preserving insertion order. Preserve all unknown settings fields and update only `piFilesWidgetOverlay.pinnedRoots`; create a minimal settings JSON when absent, but never overwrite malformed JSON—show a write-error notice instead. Append accessible pinned roots after command roots for every `/readfiles` invocation, with normalized de-duplication; without explicit command roots, `ctx.cwd` remains first. Retain inaccessible pinned paths in settings, skip them at startup, and show a brief notice. Unpinning the current root must not re-root the open overlay; it only removes the path from future root lists and after switching away from the temporary current root. Refresh the `Tab` / `Shift-Tab` root list immediately, mark pinned root labels with an accent `*`, and show transient pinned/unpinned or settings-write-error notices. Colliding directory labels should include a parent segment. Do not persist browse positions or add dependencies.
